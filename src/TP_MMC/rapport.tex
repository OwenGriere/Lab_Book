...
\begin{titlepage}
  ...
  TP Méthode de Métropolis Monte-Carlo\par}
  \vspace*{\fill}
...
\section{Résultats sur un profil énergétique généré par une unique SEED}
...
On remarque que plus la température augmente plus il est facile pour la simulation d'atteindre les extremums d'énergie, ce qui est 
relativement logique compte tenu du modèle d'exclusion du critère de Metropolis. Mais la température a aussi pour effet de garder la conformation
au fond d'un puit de potentiel et de rendre compliqué la sortie d'un puit par l'acceptation d'une conformation plus instable.

\noindent On peut egalement augmenter le nombre de puits de notre profil énergétique comme vu ci-dessous, on remarque que sur cet exemple les puits les plus
profonds sont bien repéré et atteint par la méthode MMC. (Ref : Figure~\ref{fig:20puits})

\begin{figure}[H]
  \centering
  \begin{minipage}{0.48\textwidth}
    \centering
    \includegraphics[width=\textwidth]{20puits_2D.png}
    \caption{Visualisation en 2D du profil énergétique avec la trajectoire obtenue grâce à MMC pour 300 pas et 20 puits}
    \label{fig:3Dplot}
  \end{minipage}\hfill
  \begin{minipage}{0.48\textwidth}
    \centering
    \includegraphics[width=\textwidth]{20puits.png}
    \caption{Visualisation en 3D du profil énergétique avec la trajectoire obtenue grâce à MMC pour 300 pas et 20 puits}
    \label{fig:20puits}
  \end{minipage}
\end{figure}

\newpage
\section{1000 expériences de MMC avec le même profil énergétique}

Ensuite, s'interroge sur l'ecart entre le ground truth et notre MMC, pour comprendre cet écart on commence par choisir une conformation unique 
du profil énergétique avec une SEED puis on répète l'expérience 1000 fois en récupérant l'ecart entre le ground truth et nos expériences pour
finir par en calculer la moyenne et l'ecart-type.
...
\begin{table}[h]
  ...
  \hline
  \end{tabular}
  \caption{Statistiques des ecarts entre le ground truth et l'expérience}
  \label{tab:exemple}
\end{table}

\noindent Voici un histogramme de la distribution de ces écarts (Figure~\ref{fig:hist_1000}). De plus, on observe la carte dans l'espace (X, Y)
des conformations les plus stables trouvé par les 1000 expériences de MMC, on remarque que les zones de puits sont plus peuplé par les 
points définis précédemment. On peut apercevoir cette réalité par densité (effet sur la couleur rouge plus vive des points du puits)
(Figure~\ref{fig:minimal_1000}). \\

\begin{figure}[H]
  \centering
  \begin{minipage}{0.48\textwidth}
    \centering
    \includegraphics[width=\textwidth]{hist_1000.png}
    \caption{Histrogramme normalisé autour de [0;1], de la distribution des ecarts entre le ground truth et l'expérience}
    \label{fig:hist_1000}
  \end{minipage}\hfill
  \begin{minipage}{0.48\textwidth}
    \centering
    \includegraphics[width=\textwidth]{min_1000.png}
    \caption{Visualisation en 3D du profil énergétique avec la meilleure conformation obtenu par MMC pour 1000 expériences}
    \label{fig:minimal_1000}
  \end{minipage}
\end{figure}


\noindent De plus, sur ces 1000 expériences de Métropolis Monte Carlo nous testons la précision de la méthode en définissant 
un seuil entre 0 et 1, correspondant à un pourcentage, pour lequel on considère que la méthode a atteint la conformation la plus stable.
Le critère de validation est donc définit comme suit \(|\Delta E_{Théorique-Expérience}| \leq Seuil\)
...
\begin{table}[h]
  ...
  \hline
  \end{tabular}
  \caption{Pourcentage d'expérience MMC réussites}
  \label{tab:exemple}
\end{table}


\newpage
\section{10 000 expériences de MMC avec des profils énergétique différents}

Cet histogramme pour 10 000 expériences de MMC avec des profils énergétique tous différents, montre un Pourcentage de réussite de 71 \% pour un seuil à 5 \%. 
La distribution est plus nette et semble correspondre a une distribution de Boltzmann.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.80\textwidth]{hist.png}
  \caption{Histrogramme normalisé autour de [0;1], de la distribution des ecarts entre le ground truth et 
  l'expérience pour 10 000 expérience de MMC avec des profils énergétique tous différents}
  \label{fig:hist}
\end{figure}

\chapter{Discussion}

Les résultats de la MMC montrent que celle-ci a bien été implémenté et semble fonctionner correctement en s'approchant prés de la valeur
minimal de potentiel correspondant a la conformation la plus stable. On peut voir aussi que cette méthode est capable de trouver la plupart
des puits de potentiel en un seul passage si le nombre de pas choisit est suffisement grand. Le défaut ici est que la localisation des puits 
de potentiel peut nous empecher de les croiser notament a cause des effets de bords qui empeche d'acceder au puits car trop pret d'un déplacement
illégal. \\

\noindent L'autre critique notable est l'efficacité des parametres implémentés qui ne sont pas prouver par isolation de ces mêmes paramètres. En effet,
le scalaire de dimunition de l'espace de selection d'un nombre aléatoire. \\

\noindent Pour finir, la réalisation d'une MMC est grandement dépendante des paramètres choisit au préalable, je pense que mes paramètres ne sont pas 
encore assez finement choisit pour optimiser complètement la MMC même si les résulats semble ne pas être mauvais notamments sur les 10 000 avec des 
profils énergétique différents.
