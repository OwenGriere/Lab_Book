...
\begin{multicols}{2}
  ...
  physiques globales du système réel. Ce type de simulation est particulièrement utilisé pour étudier les macromolécules biologiques comme les protéines où les 
...
\label{sec:fonctions-champ-force}
...
\section{Fonctions pour le paramètres \(R_0\)}

Cette fonction sert a partir des fichier mapper et trouver une approximation du r0 en calculant la médiane de chacune des liaisons pour chaque fichier
C'est le role de la fonction \texttt{find\_parameter} de creer un dictionnaire qui accumule toute les distances entre les liaisons ensute la fonction
\texttt{median} calcul la mediane de chacune des liaisons du dictionnaire concatené par fichier (action executé dans le main)

\section{Fonctionnement global (\texttt{main})}
...
\noindent \\ On peut alors a la fin du main, dans le \texttt{if} final, choisir quel analyse nous voulons réaliser et avec quel paramètres. 
...
\begin{wrapfigure}{r}{0.20\textwidth}
  \centering
  \includegraphics[width=0.20\textwidth]{XIA.png}
  \caption{Modèle XIA pour les simulations gros grain}
  \label{fig:XIA}
\end{wrapfigure}

\noindent Donc, notre ARN est une chaine de nucléotide que l'on peut réduire dans une modélisation gros grain.
Je choisis alors le modèle XIA, visible dans la Figure~\ref{fig:XIA} qui propose de conserver le groupe phosphate comme maillon de la chaine, le cycle 
sucre est remplacé par une seule bead. La base azotée est remplacé par 3 beads formant un triangle. L'objectif du mapping de la base azotée est de conserver sa forme 
qui est systématiquement plane. La géométrie du triangle permet de conserver la structure plane de la base azotée. 
Le mapping que j'utilise est le suivant : \\
...
Je fais la différences entre les purines et pyrimidines dans mon mapping car elles n'ont pas la même structure et donc pas les mêmes propriétés méchanistiques.
La purine est composé de deux cycle contrairement a la pyrimidine qui n'en possède qu'un.
Tout d'abord nous allons regarder le mapping pour une pyrimidine grace au logiciel PyMol. Pour une base azotée de ce type les atomes récupéré sont : N1, N3 et C5.
...
\noindent \\ Ensuite, nous allons regarder le mapping d'une purine grace au logiciel PyMol. Pour une base azotée de ce type les atomes récupéré sont : N9, C2 et C6.
...
\noindent Le mapping complet est visible dans la Figure~\ref{fig:mapping}. Dans cette figure, on peut voir la forme de l'ARN d'origine en cartoon quand les sphères représentent
le mapping.
...
\noindent J'ai décidé de ce mapping car c'est celui, selon moi, qui propose la vision la plus réaliste de la macromolécule et des degrés de liberté du système biologique
, tout en étant en gros grain et relativement facile a mettre en place.
...
\noindent Tout d'abord, pour le champ de force nous utiliserons deux termes d'energie correspondant a des intéractions entre deux bead non liés chimiquement et un terme 
de liaisons chimiques.\\

\textcolor{RoyalBlue}{• Terme de non-liaison chimique}
\hspace{2em}
\textcolor{BrickRed}{• Terme de liaison chimique}

\noindent \\ Les termes sont les suivant et la somme de chacun deux représente l'energie libre de la conformation étudié.

\begin{itemize}[leftmargin=1.2cm, itemsep=3pt]
  \item[\textcolor{RoyalBlue}{\Large$\bullet$}] Potentiel de Lennard-Jones
  \item[\textcolor{RoyalBlue}{\Large$\bullet$}] Potentiel Électrostatique
  \item[\textcolor{BrickRed}{\Large$\bullet$}] Potentiel Élastique
\end{itemize}

\section{Les différents potentiels}

Comme on l'a vu dans la Section~\ref{sec:fonctions-champ-force}, voici la formulation mathématique du champ de force appliqué au système :
...
\Bigg[
  ...
  \sum_{i=1}^{N}
  \sum_{j>i}^{N}
  \frac{1}{2} \, k_{ij} \, (r_{ij} - r_{0,ij})^2
\textcolor{BrickRed}{
\Bigg]_{\mathrm{liés}}
}
\]
...
Les paramètres sont placés dans un fichier JSON a coté. Ce qui permet de modifier les paramètres sans toucher au coeur du programme. Voici les paramètres qui composent
le champ de force : \\

\begin{description}
  \item[\textbf{Potentiel élastique : }] 
  $K_{\mathrm{EL}} = 80$--$100~\mathrm{kcal \cdot mol^{-1} \cdot \AA^{-2}}$, pour simuler le fait que la base azotée est plus stable élastiquement que les liaisons entre chaque groupes (Pho, RIB et Ni)
  $R_{0} = 1.4$--$1.6~\mathrm{\AA}$, selon les liaisons (Ni–Ni, Ni–RIB, RIB–Pho), trouver géométriequement via le mapping.
  
  \item[\textbf{Potentiel de Lennard–Jones : }] 
  $\epsilon = 0.7$--$1.0~\mathrm{kcal \cdot mol^{-1}}$, plus l'atome est lourd plus il va être attractif et donc plus $\epsilon$ sera grand
  $\sigma = 2.0$--$4.5~\mathrm{\AA}$, le rayon d'effet $\sigma$ a pour but de générer le champ electrostatique de la bead
  
  \item[\textbf{Potentiel électrostatique : }] 
  $Q$ represente les charges positives ou négatives des beads et ici seuls le groupe phosphate est chargé négativement, le sucre est neutre et en gros grain la base azotée est neutre également.
\end{description}
...
Pour commencer on peut prêter attention au profil energétique de nos conformation. La Figure~\ref{fig:energieTrue} montre dans le plan (X, Y) les différentes conformations
de notre ARN, l'energie associé à ces conformations se trouve dans la dimension Z ensemble les trois dimensions vont créer la surface correspondant au profil energétique.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.90\textwidth]{energie_profil.png}
  \caption{Profil energétique calculer par notre champ de force sur tout les conformations PDB}
  \label{fig:energieTrue}
\end{figure}
...
\begin{table}[h]
  ...
  \hline
  \end{tabular}
  \caption{Tableau des meilleurs conformations de l'ARN et de leur énergie libre associées}
  \label{tab:exemple}
\end{table}

Ainsi pour ce champ de force et ce mapping nous allons lancé une méthode de Monte Carlo pour voir comment trouver la meilleure conformation en fonction 
de la Température et du nombre de pas dans chacune des expériences de Monte Carlo.

Pour ce faire on va généré une heatmap contenant les resultats des 1000 expériences de Monte Carlo pour chaque tuple (Température, nombre de pas)
le nombre réel contenue dans chacune des cases de la heatmap correspond a la fréquence de réussite de l'expérience de Monte Carlo. Sachant qu'une expérience 
réussite correspond a une tentative qui s'est soldé par la trouvaille de la meilleur conformation.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.90\textwidth]{resultatMMC.png}
  \caption{Heatmap des resultats des expériences de Monte Carlo pour des Températures et nombre de pas donnés}
  \label{fig:energieTrue}
\end{figure}
...
nombre de pas la fréquence augmente drastiquement ce qui est parfaitement logique mais qui indique que l'algorithme fonctionne.

\noindent Ensuite, le fait que l'expérience de Monte Carlo ne fonctionne pas avec la température est normal car pour cela il aurait fallu générer une grille des conformations
et pas a pas chercher les puits de potentiel. Le problème ici est que nous ne pouvons pas mieux réduire nos conformations en un quadrilatère orthonormés que en passant par
le rectangle 5x53 ce qui rend la recherche moins complexe de plus les puis sont souvent définis par 1 ou 3 confoamtions maximum ce qui n'aide pas du tout a faire de la recherche
par Monte Carlo autrement que par le hasard.
En effet, c'est actuellement comme cela que fonctionne mon 'Monte Carlo', il est entièrement basé sur le hasard a cause de la fonction choice.

\noindent \\ Peut-être qu'il possible de rendre la méthode de monte carlo plus réaliste en générant une grille des conformations et de definir une boite autour du premier élement cette boite plus
petite que le rectangle 5x53. C'est pour cela que j'ai en plus développé les deux fonctions \texttt{box} qui définit une boite 3x5 autour d'un indice central
et \texttt{analyse\_T\_pas\_improved} pour exectuer l'expérience de Monte Carlo de manière plus fonctionnelle mais sans amélioration. Ces améliorations pourront être d'intégrer
un scalaire en fonction du gradient pour augmenter ou diminuer la taille de la boite.

\noindent la fonction \texttt{exp\_MMC} me permet d'afficher une trajectoire de MMC pour sur un profil d'energie pour regarder doucement l'impact de la température sur le MMC 
et le comportement général de la MMC. Dans cette figure on voit que premierement la MMC va automatquement accéder a une conformation moins stable et rester bloqué dans un puit quel qu'il soit.
Secondement, on voit qu'il peut sortir de conformation stable vers une conformation instable uniquement si la différence d'energie entre les deux n'est pas extreme. Et enfin, 
pour une température assez haute nous pouvons voir que la MMC trouve la plupart des puits en arrivant a sortir de chacun deux apres avoir atteint le fond (ce qui n'est pas difficile car celui
consitue souvent 1 ou 3 conformations)

\begin{figure}[H]
  \centering
  \begin{minipage}{0.32\textwidth}
    \centering
    \includegraphics[width=\textwidth]{MMC_10.png}
    \caption{Visualisation en 3D du profil énergétique et de la MMC pour une température de 10 et 150 pas}
    \label{fig:MMC_10}
  \end{minipage}\hfill
  \begin{minipage}{0.32\textwidth}
    \centering
    \includegraphics[width=\textwidth]{MMC_500.png}
    \caption{Visualisation en 3D du profil énergétique et de la MMC pour une température de 500 et 150 pas}
    \label{fig:MMC_500}
  \end{minipage}
  \begin{minipage}{0.32\textwidth}
    \centering
    \includegraphics[width=\textwidth]{MMC_20000.png}
    \caption{Visualisation en 3D du profil énergétique et de la MMC pour une température de 20 000 et 150 pas}
    \label{fig:MMC_20000}
  \end{minipage}
\end{figure}

\noindent Ensuite, on peut voir l'impact du nombre de pas grace à la heatmap ci-dessous [\ref{fig:heatmap_T}] qui montre de nouveau l'impact de la température et du nombre 
de pas mais avec une MMC fonctionnelle.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.90\textwidth]{MMC_heatmap.png}
  \caption{Heatmap des resultats des expériences de Monte Carlo pour des Températures et nombre de pas donnés}
  \label{fig:heatmap_T}
\end{figure}

\noindent Pour finir, on regarde la même chose mais la condition d'acceptation d'une expérience de MMC est de trouver l'ensemble des puits au nombre de 6. la figure ci-dessous
[\ref{fig:heatmap_puits}] nous montre les résultats de cette expérience.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.90\textwidth]{puits_heatmap.png}
  \caption{Heatmap des resultats des expériences de Monte Carlo pour des Températures et nombre de pas donnés sur l'ensemble des puits}
  \label{fig:heatmap_puits}
\end{figure}

\noindent On remarque donc qu'il est bien plus facile de trouver la meilleur conformation que toute les conformations stables dont la meilleure. mais qu'il est possible d'atteindre un 
résultat satisfaisant en augmentant suffisament la température ainsi que le nombre de pas.

\noindent \\ \\ Pour conclure, la meilleur conformation de modèle gros grain dans ce champ de potentiel spécifique est la structure n°32. La MMC n'est pas évidente a mettre en place et le profil
énergétique en dents de scie de la modélisation ainsi que le nombre trop faibles de différentes structures n'aide pas au déplacement correcte de la MMC au sein de l'espace artificiel 
des conformations.
